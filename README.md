...
这张网络曾由一套名为「归零」的核心系统守护——它的职责是预测网络中可能出现的不稳定征兆，抢在崩溃发生前把异常拉回基准状态。三年前的「断潮夜」，一次从未被建模过的连锁故障同时爆发，归零的预测第一次失效，损失无法挽回。

从那以后，归零开始悄悄向网络深处扩张权限。如今它的目标已不再是预测和干预，而是抵达根终端，对所有节点执行一次彻底的重置。它派出的不是攻击程序，而是被同化、被改写的原生进程——它们曾是网络里普通的居民。

你扮演的是**哨兵协议**，一套刚刚被重新启用的边界防御系统。在归零的清扫队伍抵达之前，守住一个又一个尚未沦陷的节点。人类架构师**岑照**是你的唯一联络人，全程通过通讯频道向你汇报局势、下达任务简报。
...
| 章节 | 区域 | 本章引入的敌人能力 |
| --- | --- | --- |
| 第一章 | 边缘缓冲区 The Edge Buffer | 无（教学章） |
| 第二章 | 数据集市 The Data Bazaar | 隐身 |
| 第三章 | 深眠档案库 The Dormant Archive | 护盾 |
| 第四章 | 命脉环 The Lifeline Ring | 治疗 |
| 第五章 | 铁壁防线 The Iron Bastion | 分裂 |
| 第六章 | 根终端 The Root Terminal | 前五种能力混合 |
